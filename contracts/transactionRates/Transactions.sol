...
contract Transactions is Rates{
    ...
    function addAgentRateTransaction(address _patreonKey, address _sponsorKey, uint _sponsorRate, address _agentKey, uint _rateKey, uint256 _transAmount)
    public onlyOwnerOrRootAdmin(msg.sender) {
        uint256 transactionTimeStamp = block.timestamp;
        // console.log("ADDING RATE REC = ",_rateKey, "ADDING TRANSACTION = ",_transAmount);
        addAgentRate(_patreonKey, _sponsorKey, _sponsorRate, _agentKey, _rateKey);
    //    AgentRateStruct storage rateRec = getRateRecordByKeys(_patreonKey, _sponsorKey, _agentKey, _rateKey);
        ...
        AgentRateStruct storage rateRec = agentRec.rateMap[_rateKey];
